...
class TaskMcscrController extends Controller
{
    ...
    public function index()
    {
        ...
        $searchQuery = request('query');
            ...
            return [
                ...
                'total' => TaskMcscr::count(),
                'terminado' => TaskMcscr::where('task_mcscr_status_id',2)->count(),
                'programado' => TaskMcscr::where('task_mcscr_status_id',4)->count(),
                'execucao' => TaskMcscr::where('task_mcscr_status_id',3)->count(),
            ];
    }
    ...
    public function calendar(){
        ...
        $tasks = TaskMcscr::get()->map(function($task){
            return [
                'id'=>'taskk'.$task->id,
        ...
        $mcscr =  Mcscr::get()->map(function($mcscr){
            return [
                'id'=>'mcscr'.$mcscr->id,
    ...
    public function detailcalendar($parms){
        ...
        if($parms[0] == 't'){
            $event = TaskMcscr::
                ...
                ->with('requestitens.technician')
                ->with('requestitens.department')
                ->where('task_mcscr_id',$string)->get();
