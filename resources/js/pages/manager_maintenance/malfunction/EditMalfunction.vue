...
const getData = () => {
  axios.get(`/malfunctions/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/malfunctions/${retrievedData.value.id}`,values).then((response)=>{

    // admins.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/manager/maintenance/malfunctions' });
