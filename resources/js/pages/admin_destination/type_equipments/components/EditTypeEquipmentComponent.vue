...
const getData = () => {
  axios.get(`/typeequipmentcomponent/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/typeequipmentcomponent/${retrievedData.value.id}`,values).then((response)=>{

    // admins.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/admin/type_equipments/'+retrievedData.value.type_equipment_id });
    toastr.success('Componente do Tipo de Equipamento editado com sucesso');
                                                    ...
                                                    <Form @submit="editFunction" :validation-schema="schema" v-slot="{ errors }">
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="mb-3 col-md-6">
                                                                            ...
                                                                            <Field type="hidden" class="form-control" :class="{'is-invalid':errors.type_equipment_id}" name="type_equipment_id" id="type_equipment_id" v-model="retrievedData.type_equipment_id"/>
