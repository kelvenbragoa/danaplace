<template>
    <div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="h3 mb-0">Registrar Nova Falta</h1>
                <p class="text-muted">Registre faltas, atrasos ou saídas antecipadas de técnicos</p>
            </div>
            <router-link 
                to="/admin/absences" 
        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <h6 class="text-primary">Falta (dia completo)</h6>
                        <p class="small text-muted mb-3">
                            Ausência por um dia inteiro de trabalho. 
                            Normalmente 8 horas para jornada padrão.
                        </p>
                    </div>
                    <div class="col-md-4">
                        <h6 class="text-warning">Atraso</h6>
                        <p class="small text-muted mb-3">
                            Chegada após o horário estabelecido. 
                            Informe as horas de atraso.
                        </p>
                    </div>
                    <div class="col-md-4">
                        <h6 class="text-info">Saída Antecipada</h6>
                        <p class="small text-muted mb-3">
                            Saída antes do fim do expediente. 
                            Informe as horas não trabalhadas.
                        </p>
                    </div>
                </div>
                <div class="alert alert-warning">
                    <vue-feather type="alert-triangle" size="16" class="me-2"></vue-feather>
                    <strong>Atenção:</strong> As faltas registradas aqui serão consideradas no cálculo do processamento salarial após aprovação.
                </div>
            </div>
        </div>
    </div>
...
const getTechnicians = async () => {
    try {
        const response = await axios.get('/absences/create');
...
const onSubmit = async (values) => {
    ...
    try {
        await axios.post('/absences', values);
        toastr.success('Falta registrada com sucesso!');
        ...
        if (error.response?.data?.errors) {
            ...
            toastr.error('Erro ao registrar falta');
        }
    } finally {
        loading.value = false;
    }
