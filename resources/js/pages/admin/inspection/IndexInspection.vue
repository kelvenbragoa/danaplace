...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1,destinationfilter) => {
  axios.get(`/inspections?page=${page}`,
      {
        params:{
       ...
       .then((response)=>{
        retriviedData.value = response.data.inspection;
...
axios.delete(`/inspections/${dataIdBeingDeleted}`)
.then(()=>{
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela das Inspeção do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/inspections/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                        ...
                                        <form class="d-none d-sm-inline-block mt-3">
                                            <div class="input-group input-group-navbar">
                                                <select name="date_range" @change="getData(1,destinationfilter)" v-model="destinationfilter" class="form-control">
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>{{ actualData.id }}</td>
                                                        <td>
                                                            <span v-if="actualData.inspection_status_id == 1" class="badge bg-warning">
                                                                {{ actualData.inspection_status.name}}
                                                            </span>
                                                            <span v-if="actualData.inspection_status_id == 2" class="badge bg-success">
                                                                {{ actualData.inspection_status.name}}
                                                            </span>
                                                        </td>
                                                        ...
                                                        <td>
                                                            <router-link :to="'/admin/inspections/'+actualData.id+'/edit'" v-if="actualData.inspection_status_id == 1"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <!-- <router-link :to="'/admin/inspections/'+actualData.id+'/edit'" ><vue-feather type="edit-2"></vue-feather></router-link> -->
                                                            <router-link :to="'/admin/inspections/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
