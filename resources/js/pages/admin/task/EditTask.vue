...
const getData = () => {
  axios.get(`/tasks/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/tasks/${retrievedData.value.id}`,values).then((response)=>{
...
<template>
    <div v-if="!loadingDiv">
        ...
        <h1 class="h3 mb-3">Actividade </h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Actividade: {{ retrievedData.name }}</h5>

                                        <router-link to="/admin/tasks" class="btn btn-pill btn-primary mt-3"><vue-feather type="arrow-left"></vue-feather>Voltar</router-link> 
