...
const getData = () => {
  axios.get(`/meetingtype/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/meetingtype/${retrievedData.value.id}`,values).then((response)=>{

    // admins.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/admin/meetingtype' });
    toastr.success('Tipo de Reunião editada com sucesso');
...
<template>
    <div v-if="!loadingDiv">
        ...
        <h1 class="h3 mb-3">Tipo de Reunião</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tipo de Reunião: {{ retrievedData.name }}</h5>

                                        <router-link to="/admin/meetingtype" class="btn btn-pill btn-primary mt-3"><vue-feather type="arrow-left"></vue-feather>Voltar</router-link> 
