                                ...
                                <tr v-for="expense in expenses.data" :key="expense.id">
                                    <td>
                                        ...
                                        <div v-if="expense.vendor_name" class="text-muted small">
                                            <vue-feather type="home" size="12" class="me-1"></vue-feather>
                                            {{ expense.vendor_name }}
                                        </div>
                                    </td>
                                    <td>
...
const getData = async (page = 1) => {
    ...
    try {
        ...
        if (response.data) {
            expenses.value = response.data.expenses || { data: [], current_page: 1, last_page: 1 };
            ...
            expenses.value = { data: [], current_page: 1, last_page: 1 };
            statistics.value = {
                total_expenses: 0,
                total_amount: 0,
                pending_count: 0,
...
const approveExpense = async () => {
    ...
    loading.value = true;
    try {
        await axios.post(`/expenses/${selectedExpense.value.id}/approve`);
...
const payExpense = async () => {
    ...
    loading.value = true;
    try {
        await axios.post(`/expenses/${selectedExpense.value.id}/pay`, {
...
const rejectExpense = async () => {
    ...
    loading.value = true;
    try {
        await axios.post(`/expenses/${selectedExpense.value.id}/reject`, {
...
const confirmDelete = async () => {
    ...
    loading.value = true;
    try {
        await axios.delete(`/expenses/${selectedExpense.value.id}`);
