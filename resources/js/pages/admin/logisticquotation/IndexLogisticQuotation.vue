...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/logisticquotation?page=${page}`,
      {
        params:{
       ...
       .then((response)=>{
        retriviedData.value = response.data.logisticquotation;
...
axios.delete(`/logisticquotation/${dataIdBeingDeleted}`)
.then(()=>{
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos Cotações do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/logisticquotation/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        <td>#{{ index + 1 }}</td>
                                                        <td>{{ actualData.customer == null ? actualData.customer_name : actualData.customer.customer_name}}</td>
                                                        <td>{{ actualData.tripdestination == null ? actualData.departure : actualData.tripdestination.departure}}</td>
                                                        <td>{{ actualData.tripdestination == null ? actualData.destination : actualData.tripdestination.departure}}</td>
                                                        <td>{{ actualData.typeload.name}}</td>
                                                        <td>{{ actualData.tripdestination == null ? 0 : actualData.tripdestination.total_distance}}</td>
                                                        <td>{{ moment(actualData.created_at).format('DD-MM-YYYY')}}</td>
                                                        <td>
                                                            <span v-if="actualData.status_logistic_quotation_id == 1" class="badge bg-warning">
                                                                {{ actualData.status.name}}
                                                            </span>
                                                            <span v-if="actualData.status_logistic_quotation_id == 2" class="badge bg-success">
                                                                {{ actualData.status.name}}
                                                            </span>
                                                            <span v-if="actualData.status_logistic_quotation_id == 3" class="badge bg-danger">
                                                                {{ actualData.status.name}}
                                                            </span>
                                                        ...
                                                        <td>
                                                            <router-link :to="'/admin/logisticquotation/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <router-link :to="'/admin/logisticquotation/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
