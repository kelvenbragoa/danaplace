...
const getData = () => {
  axios.get(`/energyconsumption/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/energyconsumption/${retrievedData.value.id}`,values).then((response)=>{

    // admins.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/admin/energyconsumption' });
    toastr.success('Registo editada com sucesso');
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Consumo de Energia: {{ retrievedData.value }}</h5>

                                        <router-link to="/admin/energyconsumption" class="btn btn-pill btn-primary mt-3"><vue-feather type="arrow-left"></vue-feather>Voltar</router-link> 
                                            ...
                                            <div class="col-xl-12 col-xxl-12 d-flex">
                                                <div class="w-100">
                                                    <Form @submit="editFunction" :validation-schema="schema" v-slot="{ errors }">
                                                        ...
                                                        <div class="row">
                                                            <div class="mb-3 col-md-12">
                                                                <label class="form-label" for="equipment_id">Medidor de Distancia:</label>
                                                                <span class="form-control">{{ retrievedData.distance_control.name }}</span>
                                                            </div>
                                                        </div>
