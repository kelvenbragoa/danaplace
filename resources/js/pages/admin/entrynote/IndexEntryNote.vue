...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/entrynotes?page=${page}`,
      {
        params:{
...
// axios.post('/entrynotes',values).then((response)=>{

//   entrynotes.value.unshift(response.data);
...
axios.delete(`/entrynotes/${dataIdBeingDeleted}`)
.then(()=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Notas de Entrada</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos Notas de Entrada do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/entrynotes/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 

                                        <br>

                                        <form class="d-none d-sm-inline-block mt-3">
                                            <div class="input-group input-group-navbar">
                                                <input type="text" class="form-control" v-model="searchQuery" placeholder="Procurar ref,documento..." aria-label="Search">
                                            </div>
                                        </form>
								    </div>
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>{{ actualData.document_ref}}</td>
                                                        <td>{{ actualData.serie}}</td>
                                                        <td>{{ actualData.stockcenter.name}}</td>
                                                        <td>{{ actualData.supplier.name}}</td>
                                                        <td>{{ moment(actualData.created_at).format('DD-MM-YYYY H:mm')}}</td>
                                                        
                                                        <td>
                                                            <!-- <router-link :to="'/admin/entrynotes/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link> -->
                                                            <router-link :to="'/admin/entrynotes/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
