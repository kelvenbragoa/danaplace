...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1,destinationfilter) => {
  axios.get(`/meetingparticipant?page=${page}`,
      {
        params:{
       ...
       .then((response)=>{
        retriviedData.value = response.data.meetingparticipant;
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos Funcionários do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        <td>#{{ index + 1 }}</td>
                                                        <td>{{ actualData.firstName}} {{ actualData.lastName}}</td>
                                                        <td>{{ actualData.email}}</td>
                                                        <td>{{ actualData.mobile}}</td>
                                                        <td>{{ actualData.role.name}}</td>
                                                        <td>{{ actualData.destination == null ? 'N/A' : actualData.destination.name}}</td>
                                                        <td>{{ actualData.taskdone.length}}</td>
                                                        <td>{{ actualData.tasknotdone.length}}</td>
                                                        <td>
                                                            <!-- <router-link :to="'/admin/users/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link> -->
                                                            <router-link :to="'/admin/meetingparticipant/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
