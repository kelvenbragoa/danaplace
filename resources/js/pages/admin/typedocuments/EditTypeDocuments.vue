...
const getData = () => {
  axios.get(`/typedocuments/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{

        loadingDiv.value=false;
        retrievedData.value = response.data.typedocument;
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/typedocuments/${retrievedData.value.id}`,values).then((response)=>{

    // admins.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/admin/typedocuments' });
    toastr.success('Tipo de Documentos editada com sucesso');
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tipo de Documentos: {{ retrievedData.name }}</h5>
