...
axios.delete(`/meeting/${dataIdBeingDeleted}`)
.then(()=>{
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela das Reunião do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/meeting/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Tarefas a espera de aprovação</th>
                                                        <th>Ações</th>
                                                    </tr>
                                                </thead>
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>{{ actualData.subject}}</td>
                                                        <td>{{ actualData.typemeeting.name}}</td>
                                                        <td>{{ actualData.participants.length}}</td>
                                                        <td>{{ actualData.completed_tasks.length}}</td>
                                                        <td>{{ actualData.incompleted_tasks.length}}</td>
                                                        <td>{{ actualData.waiting_approval_tasks.length}}</td>
                                                        <td>
                                                            <router-link :to="'/admin/meeting/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <router-link :to="'/admin/meeting/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
