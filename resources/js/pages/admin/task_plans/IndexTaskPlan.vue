...
axios.get(`/taskplans/${dataIdBeingCopied}/copy`)
.then((response)=>{
...
axios.delete(`/taskplans/${dataIdBeingDeleted}`)
.then(()=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Plano de Atividades</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos planos de atividades do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/taskplans/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th>Nome do Plano de Tarefa</th>
                                                        <th>Atividades Associadas</th>
                                                        <th>Equipamentos/Ativos Associadas</th>
                                                        <th>Ações</th>
                                                    </tr>
                                                </thead>
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>{{ actualData.taskplantasks.length }}</td>
                                                        <td>{{ actualData.equipments.length }}</td>
                                                       
                                                        <td>
                                                            <router-link :to="'/admin/taskplans/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <router-link :to="'/admin/taskplans/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
   ...
   <div class="modal" id="copyModalTask" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            Ao confirmar será copiado todos os registros deste plano atividade.
        </div>
        <div class="modal-footer">
