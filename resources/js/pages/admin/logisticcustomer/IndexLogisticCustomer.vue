...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/logisticcustomer?page=${page}`,
      {
        params:{
       ...
       .then((response)=>{
        retriviedData.value = response.data.logisticcustomer;
...
axios.delete(`/logisticcustomer/${dataIdBeingDeleted}`)
.then(()=>{
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <router-link to="/admin/logisticcustomer/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>{{ actualData.customer_email}}</td>
                                                        <td>{{ actualData.customer_mobile}}</td>
                                                        <td>{{ actualData.customer_address}}</td>
                                                        <td>{{ actualData.customer_nuit}}</td>
                                                        <td>{{ moment(actualData.created_at).format('DD-MM-YYYY')}}</td>
                                                        <td>
                                                            <router-link :to="'/admin/logisticcustomer/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <router-link :to="'/admin/logisticcustomer/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
