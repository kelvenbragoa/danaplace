...
const getData = () => {
  axios.get(`/shifts/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/shifts/${retrievedData.value.id}`,values).then((response)=>{

    // admins.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/admin/shifts' });
                                                    ...
                                                    <Form @submit="editFunction" :validation-schema="schema" v-slot="{ errors }">
                                                        ...
                                                        <div class="row">
                                                            <div class="mb-3 col-md-12">
                                                                ...
                                                                <Field type="date" class="form-control" :class="{'is-invalid':errors.date}" name="date" v-model="retrievedData.date" id="date" placeholder="Nome"/>
