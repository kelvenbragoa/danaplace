...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/stocktransfers?page=${page}`,
      {
        params:{
...
axios.delete(`/stocktransfers/${dataIdBeingDeleted}`)
.then(()=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Transferências de Stock</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela das Transferências de Stock do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/stocktransfers/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 

                                        <br>

                                        <form class="d-none d-sm-inline-block mt-3">
                                            <div class="input-group input-group-navbar">
                                                <input type="text" class="form-control" v-model="searchQuery" placeholder="Procurar ref..." aria-label="Search">
                                            </div>
                                        </form>
								    </div>
                                            ...
                                            <table class="table table-striped">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th>Ref</th>
                                                        <th>Centro de Stock de Origem</th>
                                                        <th>Centro de Stock de Destino</th>
                                                        <th>Data</th>
                                                        <th>Ações</th>
                                                    </tr>
                                                </thead>
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>{{ actualData.stockcenterorigin.name}}</td>
                                                        <td>{{ actualData.stockcenterdestination.name}}</td>
                                                        <td>{{ moment(actualData.created_at).format('DD-MM-YYYY H:mm')}}</td>
                                                        
                                                        <td>
                                                            <!-- <router-link :to="'/admin/stocktransfers/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link> -->
                                                            <router-link :to="'/admin/stocktransfers/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
