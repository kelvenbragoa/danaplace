...
const getStats = () => {
    axios.get('/admin/mortality/dashboard-stats').then((response) => {
        stats.value = response.data;
    });
};

const getData = async (page = 1) => {
    axios.get(`/admin/mortality?page=${page}`, {
        params: {
...
const deleteData = () => {
    ...
    axios.delete(`/admin/mortality/${dataIdBeingDeleted.value}`)
        .then(() => {
    ...
    <div v-if="!loadingDiv">
        ...
        <div class="row mb-3">
            <div class="col-md-3">
                <div class="card">
                    <div class="card-body">
                        <h6 class="text-muted">Mortes Hoje</h6>
                        <h3>{{ stats.deaths_today || 0 }}</h3>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card">
                    <div class="card-body">
                        <h6 class="text-muted">Mortes no Mês</h6>
                        <h3>{{ stats.deaths_month || 0 }}</h3>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card">
                    <div class="card-body">
                        <h6 class="text-muted">Registos no Mês</h6>
                        <h3>{{ stats.records_month || 0 }}</h3>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card">
                    <div class="card-body">
                        <h6 class="text-muted">Necropsias no Mês</h6>
                        <h3>{{ stats.necropsies_month || 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Registos de mortalidade. {{ retriviedData.total }} registros encontrados.</h5>

                        <router-link to="/admin/mortalidade/create" class="btn btn-pill btn-primary mt-3">
                        ...
                        <form class="d-none d-sm-inline-block mt-3">
                            <div class="input-group input-group-navbar">
                                <input type="text" class="form-control" v-model="searchQuery" placeholder="Procurar lote ou causa..." aria-label="Search">
                            </div>
                        </form>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody v-if="retriviedData.data.length > 0">
                                    <tr v-for="(actualData, index) in retriviedData.data" :key="actualData.id">
                                        ...
                                        <td class="text-danger">{{ actualData.quantity }}</td>
                                        <td>{{ actualData.probable_cause || '-' }}</td>
                                        <td>
                                            <span class="badge bg-success" v-if="actualData.necropsy_performed">Sim</span>
                                            <span class="badge bg-secondary" v-else>Não</span>
                                        </td>
                                        <td>{{ actualData.responsible?.name || '-' }}</td>
                                        <td>
                                            <router-link :to="'/admin/mortalidade/' + actualData.id + '/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                            <router-link :to="'/admin/mortalidade/' + actualData.id"><vue-feather type="eye"></vue-feather></router-link>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Ao apagar este registo, as aves serão repostas no lote.
                    </div>
