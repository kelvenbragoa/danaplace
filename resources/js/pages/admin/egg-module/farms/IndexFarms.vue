...
const getData = async (page = 1) => {
    axios.get(`/admin/farms?page=${page}`, {
        params: {
...
const deleteData = () => {
    ...
    axios.delete(`/admin/farms/${dataIdBeingDeleted.value}`)
        .then(() => {
...
const toggleStatus = (farm) => {
    loadingToggleStatus.value = farm.id;

    axios.post(`/admin/farms/${farm.id}/toggle-status`)
        .then((response) => {
            const index = retriviedData.value.data.findIndex(item => item.id === farm.id);
            if (index !== -1) {
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Granjas</h1>

        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Tabela das granjas do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                        <router-link to="/admin/granjas/create" class="btn btn-pill btn-primary mt-3">
                        ...
                        <form class="d-none d-sm-inline-block mt-3">
                            <div class="input-group input-group-navbar">
                                <input type="text" class="form-control" v-model="searchQuery" placeholder="Procurar nome, NUIT ou email..." aria-label="Search">
                            </div>
                        </form>
                    </div>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody v-if="retriviedData.data.length > 0">
                                    <tr v-for="(actualData, index) in retriviedData.data" :key="actualData.id">
                                        ...
                                        <td>{{ actualData.tax_id }}</td>
                                        <td>{{ actualData.phone || '-' }}</td>
                                        <td>{{ actualData.email || '-' }}</td>
                                        <td>{{ actualData.houses_count }}</td>
                                        <td>
                                            <span class="badge bg-success" v-if="actualData.is_active">Ativa</span>
                                            <span class="badge bg-danger" v-else>Inativa</span>
                                        </td>
                                        <td>
                                            <router-link :to="'/admin/granjas/' + actualData.id + '/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                            <router-link :to="'/admin/granjas/' + actualData.id"><vue-feather type="eye"></vue-feather></router-link>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Ao apagar esta granja, irá apagar todos os registros relacionados a ela.
                    </div>
