...
const getData = async (page = 1) => {
    axios.get(`/admin/reject-reasons?page=${page}`, {
...
const deleteData = () => {
    ...
    axios.delete(`/admin/reject-reasons/${dataIdBeingDeleted.value}`)
        .then(() => {
...
const toggleStatus = (reason) => {
    togglingId.value = reason.id;

    axios.post(`/admin/reject-reasons/${reason.id}/toggle-status`)
        .then((response) => {
            const index = retriviedData.value.data.findIndex(item => item.id === reason.id);
            if (index !== -1) {
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Motivos de Refugo</h1>

        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Tabela de motivos. {{ retriviedData.total }} registros encontrados.</h5>

                        <router-link to="/admin/motivos-refugo/create" class="btn btn-pill btn-primary mt-3">
                        ...
                        <form class="d-none d-sm-inline-block mt-3">
                            <div class="input-group input-group-navbar">
                                <input type="text" class="form-control" v-model="searchQuery" placeholder="Procurar nome ou descrição..." aria-label="Search">
                            </div>
                        </form>
                    </div>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody v-if="retriviedData.data.length > 0">
                                    <tr v-for="(actualData, index) in retriviedData.data" :key="actualData.id">
                                        ...
                                        <td>
                                            <router-link :to="'/admin/motivos-refugo/' + actualData.id + '/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                            <router-link :to="'/admin/motivos-refugo/' + actualData.id"><vue-feather type="eye"></vue-feather></router-link>
                                            <a href="#" @click.prevent="toggleStatus(actualData)" :title="actualData.is_active ? 'Desativar' : 'Ativar'">
                                                <vue-feather :type="actualData.is_active ? 'toggle-right' : 'toggle-left'"></vue-feather>
                                            </a>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Ao apagar este motivo de refugo, registos existentes podem manter o texto antigo.
                    </div>
