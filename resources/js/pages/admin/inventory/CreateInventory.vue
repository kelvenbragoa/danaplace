...
const createRecordFunction = (values, actions) => {
    ...
    actions.resetForm();
    router.push({ path: '/admin/inventories' });
    toastr.success('Inventário criada com sucesso');
  }).catch((error)=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Inventário</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Formulário criação dos Inventários do sistema.</h5>
                                        <router-link to="/admin/inventories" class="btn btn-pill btn-primary mt-3"><vue-feather type="arrow-left"></vue-feather>Voltar</router-link>  
								    </div>
                                    <div class="card-body">
                                            <Form @submit="createRecordFunction" :validation-schema="schema" v-slot="{ errors }">
												...
												<div class="row">
													<div class="mb-3 col-md-12">
														<label class="form-label" for="name">Referência</label>
                                                                ...
                                                                <tbody v-if="stockcenterproducts.length > 0">
                                                                    <FieldArray class="form-control" name="stockcenterproducts">
                                                                        ...
                                                                        <tr v-for="(stockcenterproduct, idx) in stockcenterproducts" :key="stockcenterproduct.key">
                                                                            ...
                                                                            <td>
                                                                                <fieldset>
                                                                                    <Field type="text" class="form-control" :name="`stockcenterproducts[${idx}].quantity`" :id="`stockcenterproduct[${idx}].quantity`" placeholder="Contado"/>
